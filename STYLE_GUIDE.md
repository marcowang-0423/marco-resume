...
**目標**：正式、舒服，適合面試投遞與給主管瀏覽，避免過度科技感。

- 暖象牙底（`#f7f5f0`），非純白，視覺上更舒適
- 卡片純白浮卡，帶細陰影（`var(--shadow)`），在暖底上自然浮起
- 主色調用深青（teal），色溫與暖底和諧，不用冷色海軍藍
- 字型以 Noto Sans TC 為主，章節編號用 Lora 斜體，monospace 僅限成績欄位
...
|-----------------|-----------|-------------------------------|
| `--bg`          | `#f7f5f0` | 頁面主背景（暖象牙）           |
| `--bg-alt`      | `#eeece5` | 奇偶節交替背景、卡片表頭       |
| `--card`        | `#ffffff` | 卡片背景（浮卡，白底有陰影）   |
| `--accent`      | `#1d4555` | 深青（主色調，與暖底色溫和諧） |
| `--accent-mid`  | `#2a6a8a` | 連結、章節編號、計畫標題       |
| `--text`        | `#2c3a46` | 主要內文                       |
| `--text-muted`  | `#6b7480` | 次要文字、日期、標籤           |
| `--heading`     | `#0e1b2c` | 標題、重點文字                 |
| `--border`      | `#dbd8cf` | 所有邊框、分隔線（暖灰）       |
| `--shadow`      | （見下）  | 卡片常駐陰影                   |
| `grade-a`       | `#166534` | 優良成績（A 等）               |
| `grade-b`       | `#92400e` | 中等成績（B 等）               |

```css
--shadow: 0 1px 3px rgba(14,27,44,0.07), 0 4px 16px rgba(14,27,44,0.04);
```
...
font-family: 'Lora', serif;                /* 章節編號（斜體）*/
```

章節編號（`.section-num`）使用 Lora italic，字級 0.8rem，顏色 `var(--accent-mid)`。
...
| 卡片陰影          | `var(--shadow)`（常駐），hover 加深      |
| hover 陰影        | `0 8px 28px rgba(14,27,44,0.12)`         |
...
MW 首字母 SVG，深青底（`#1d4555`）白字，內嵌為 data URI（不需要額外圖片檔）：

```html
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><rect width='32' height='32' rx='4' fill='%231d4555'/><text x='16' y='22' text-anchor='middle' font-family='sans-serif' font-size='14' font-weight='700' fill='white'>MW</text></svg>">
```
...
<div class="hero-card">
    ...
    <div class="hero-info">
        ...
        <div class="hero-contact">              <!-- 聯絡資訊列 -->
            <a href="mailto:..." class="hero-contact-item">
                [email svg] m0983557762@gmail.com
            </a>
            <span class="hero-contact-item">[phone svg] 0909-802-001</span>
            <span class="hero-contact-item">[location svg] 臺灣，新北市</span>
            <a href="..." class="hero-contact-item">[github svg] marcowang-0423</a>
            <a href="..." class="hero-contact-item">[linkedin svg] LinkedIn</a>
        </div>
        <div class="hero-tags">...</div>
        <div class="hero-actions">
            <a href="#experience" class="btn-primary">查看經歷</a>
...
- **Hero contact**：`display: grid; grid-template-columns: repeat(3, max-content); gap: 8px 28px`，每項 icon + 值（不加文字 label），`font-size: 0.9rem`，icon `14px`，`color: var(--text-muted)`，hover → `var(--accent)`
- Hero tags：矩形小標籤，`bg-alt` 底色
- Hero actions：兩個按鈕，主要（查看經歷）/ 次要（下載 CV）
- Mobile (≤640px)：hero-contact 改 flex column 置中；name-row、affiliations 也 `justify-content: center`

---

## 區塊標題慣例

底線分隔 + 44px 短底線作為重點標記，Lora 斜體數字作為前綴：
